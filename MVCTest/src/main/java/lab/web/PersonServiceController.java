...
public class PersonServiceController {
    ...
    Logger logger1 = LogManager.getLogger(PersonServiceController.class.getName());
	...
	protected String regist(@RequestParam Map<String, Object> commandMap, person command, 
//	protected String regist(person command,
			  BindingResult errors, ModelMap model) throws Exception {
		if (errors.hasErrors()) {
			return formView;
		}
//		logger1.debug("name="+command.getName());
        Iterator<String> mapIter = commandMap.keySet().iterator();
        while(mapIter.hasNext()){
            String key = mapIter.next();
            Object value = commandMap.get( key );
            logger1.debug(key+" : "+value);
        }
